...
#tk-consent{
  ...
  width:min(660px,calc(100vw - 2rem));background:#fff;border:1px solid #ecdcd0;
  border-radius:16px;padding:1.35rem 1.5rem;box-shadow:0 18px 50px rgba(60,30,15,.22);
  display:none;
}
#tk-consent.visible{display:block;animation:tkConsentIn .45s cubic-bezier(.16,1,.3,1)}
@keyframes tkConsentIn{from{opacity:0;transform:translateX(-50%) translateY(24px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}
#tk-consent h4{font-size:1rem;font-weight:800;margin:0 0 .4rem;color:#3a2a20}
#tk-consent p{font-size:.86rem;line-height:1.55;color:#6a5648;margin:0 0 1rem}
#tk-consent p a{color:var(--coral,#e07a5f);font-weight:600}
.tk-consent-actions{display:flex;gap:.6rem;flex-wrap:wrap}
.tk-consent-actions button{
  flex:1 1 150px;padding:.75em 1.2em;border-radius:10px;border:1px solid #e0cfc2;
  font:inherit;font-weight:700;font-size:.88rem;cursor:pointer;transition:transform .2s ease
}
.tk-consent-actions button:hover{transform:translateY(-1px)}
#tk-consent-accept{background:var(--coral,#e07a5f);color:#fff;border:none}
#tk-consent-decline{background:transparent;color:#6a5648}
#tk-consent-reopen{
  position:fixed;left:1rem;bottom:1rem;z-index:9998;width:42px;height:42px;border-radius:50%;
  background:#fff;border:1px solid #e0cfc2;color:#a98c78;cursor:pointer;
  display:none;align-items:center;justify-content:center;font-size:1.1rem;box-shadow:0 6px 18px rgba(60,30,15,.15)
}
#tk-consent-reopen.visible{display:flex}
</style>
<div id="tk-consent" role="dialog" aria-modal="false" aria-label="Cookie-Einstellungen">
  <h4>🍪 Cookies &amp; Datenschutz</h4>
  <p>Wir möchten mit Google Analytics anonym verstehen, wie diese Seite genutzt wird, um sie für dich zu verbessern. Das passiert nur mit deiner Zustimmung. Mehr in der <a href="/datenschutz.html" rel="nofollow">Datenschutzerklärung</a>.</p>
  <div class="tk-consent-actions">
    <button id="tk-consent-accept" type="button">Alle akzeptieren</button>
    <button id="tk-consent-decline" type="button">Nur notwendige</button>
  </div>
</div>
<button id="tk-consent-reopen" type="button" title="Cookie-Einstellungen ändern" aria-label="Cookie-Einstellungen ändern">🍪</button>
<script>
(function(){
  "use strict";
  /* ==== TRACKING-ID (Platzhalter — echte GA4-ID vor Aktivierung eintragen) ==== */
  var GA4_ID = "G-XXXXXXXXXX";   // Google Analytics 4 Measurement-ID
  /* ============================================================================ */

  function getConsent(){
    var m = document.cookie.match(/(?:^|;\s*)tk_consent=([^;]+)/);
    return m ? m[1] : null;
  }
  function setConsent(v){
    document.cookie = "tk_consent=" + v + ";max-age=" + (60*60*24*180) + ";path=/;SameSite=Lax";
  }
  function loadTracking(){
    if (GA4_ID.indexOf("XXXX") !== -1) return; // Platzhalter: nichts laden
  ...
  var banner = document.getElementById("tk-consent");
  var reopen = document.getElementById("tk-consent-reopen");
  function show(){ banner.classList.add("visible"); reopen.classList.remove("visible"); }
  function hide(){ banner.classList.remove("visible"); reopen.classList.add("visible"); }

  document.getElementById("tk-consent-accept").addEventListener("click", function(){
    setConsent("all"); hide(); loadTracking();
  });
  document.getElementById("tk-consent-decline").addEventListener("click", function(){
    setConsent("essential"); hide();
  });
  reopen.addEventListener("click", show);

  var c = getConsent();
  if (c === "all") { hide(); loadTracking(); }
  else if (c === "essential") { hide(); }
  else { show(); }
})();
</script>
</body>
